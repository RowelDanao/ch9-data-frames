...
most_pop_f_2013 <- names_2013[names_2013$prop == max(names_2013$prop), 'name']
...
most_popular_in_year <- function(year){
  name_year <- names[names$year == year, ]
  most_popular <- name_year[name_year$prop == max(name_year$prop), 'name']
  most_popular
}

# What was the most popular female name in 1994?
most_pop_f_2014 <- most_popular_in_year(1994)
...
number_in_million('Rowel', 1997)

## Consider: what does this tell you about how easy it is to identify you with 
## just your name and birth year?
## Names and birth years act as filters that can minimize the scope of over a population of 1 million or over.